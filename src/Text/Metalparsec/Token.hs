...
unsafeScan1 :: Chunk.TokenChunk s => Chunk.TokenTag s -> Parsec s u e ()
unsafeScan1 t =
  Parsec
    ( \s _l i p u ->
...
scan1 :: Chunk.TokenChunk s => Chunk.TokenTag s -> Parsec s u e ()
scan1 t = Combinators.ensureLen 1 *> unsafeScan1 t
{-# INLINE scan1 #-}

any :: Chunk.TokenChunk s => Parsec s u e (Chunk.Token s)
any = Parsec $ \s l i p u -> case i ==# l of
  1# -> Fail#
  _ -> case Chunk.unsafeIndex# s i of
    t -> Ok# (p +# Chunk.tokenOffset# t) (i +# 1#) u t
{-# INLINE any #-}

satisfy :: Chunk.TokenChunk s => (Chunk.TokenTag s -> Bool) -> Parsec s u e (Chunk.TokenTag s)
satisfy f = Parsec $ \s l i p u -> case i ==# l of
  1# -> Fail#
  _ -> case Chunk.unsafeIndex# s i of
