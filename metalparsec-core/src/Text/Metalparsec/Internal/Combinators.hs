...
ensureLen :: Int -> BaseParsec i c s e ()
...
cut :: BaseParsec i c s e a -> e -> BaseParsec i c s e a
...
cutting :: BaseParsec i c s e a -> e -> (e -> e -> e) -> BaseParsec i c s e a
...
optional :: BaseParsec i c s e a -> BaseParsec i c s e (Maybe a)
optional p = (Just <$> p) <|> pure Nothing

-- | Convert a parsing failure to a `()`.
optional_ :: BaseParsec i c s e a -> BaseParsec i c s e ()
optional_ p = (() <$ p) <|> pure ()

-- | CPS'd version of `optional`. This is usually more efficient, since it gets rid of the
-- extra `Maybe` allocation.
withOption :: BaseParsec i c s e a -> (a -> BaseParsec i c s e b) -> BaseParsec i c s e b -> BaseParsec i c s e b
...
many_ :: BaseParsec i c s e a -> BaseParsec i c s e ()
...
some_ :: BaseParsec i c s e a -> BaseParsec i c s e ()
...
(<|>) :: BaseParsec i c s e a -> BaseParsec i c s e a -> BaseParsec i c s e a
(<|>) = (Applicative.<|>)
{-# INLINE (<|>) #-}
...
eof :: BaseParsec i c s e ()
eof = parser# $ \(Env# _ l _) p@(Ix# _ i) -> case l ==# i of
  1# -> Ok# p ()
  _ -> Fail#

fail :: BaseParsec i c s e a
fail = parser# $ \_ _ -> Fail#

-- | Return a slice consumed by a parser.
slice :: forall i chunk u e a. Chunk chunk => BaseParsec i chunk u e a -> BaseParsec i chunk u e (Chunk.ChunkSlice chunk)
...
sliceMany :: Chunk c => BaseParsec i c s e a -> BaseParsec i c s e (Chunk.ChunkSlice c)
sliceMany = slice . many_
{-# INLINE sliceMany #-}

sliceSome :: Chunk c => BaseParsec i c s e a -> BaseParsec i c s e (Chunk.ChunkSlice c)
sliceSome = slice . some_
{-# INLINE sliceSome #-}

-- | Save the parsing state, then run a parser, then restore the state.
lookahead :: BaseParsec i c s e a -> BaseParsec i c s e a
lookahead (Parsec f) = Parsec $ \e p s ->
  case f e p s of
    STR# s r -> STR# s (case r of Ok# _ a -> Ok# p a; x -> x)

-- | Convert a parsing failure to a success.
fails :: BaseParsec i c s e a -> BaseParsec i c s e ()
...
notFollowedBy :: BaseParsec i c s e a -> BaseParsec i c s e b -> BaseParsec i c s e a
...
chainPre :: BaseParsec i c s e (a -> a) -> BaseParsec i c s e a -> BaseParsec i c s e a
...
chainPost :: BaseParsec i c s e a -> BaseParsec i c s e (a -> a) -> BaseParsec i c s e a
...
-- >          <|> do{ symbol "-"; return (-) }
chainl1 :: BaseParsec i c s e a -> BaseParsec i c s e (a -> a -> a) -> BaseParsec i c s e a
...
-- application of all functions returned by @op@ to the values returned
-- by @p@.
chainr1 :: BaseParsec i c s e a -> BaseParsec i c s e (a -> a -> a) -> BaseParsec i c s e a
